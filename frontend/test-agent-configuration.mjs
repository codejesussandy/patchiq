...
async function testAgentConfiguration() {
  ...
  try {
    ...
    // Navigate directly to AgentConfiguration
    console.log('Step 6: Navigating to Agent Configuration...');
    await page.goto('http://localhost:5174/settings/agent-management/configuration');

    await page.waitForTimeout(2000);

    // Take screenshot
    console.log('Step 7: Taking screenshot...');
    const screenshotPath = path.join('.playwright-mcp', 'agent-configuration-page.png');
    ...
    console.log(`Page contains "Agent Configuration": ${html.includes('Agent Configuration')}`);
    ...
    console.log(`Page contains Allowed Bandwidth field: ${html.includes('Allowed Bandwidth')}`);
    console.log(`Page contains Agent Refresh Cycle field: ${html.includes('Agent Refresh Cycle')}`);

  } catch (error) {
    console.error('Test failed:', error.message);
    // Take error screenshot
    await page.screenshot({ path: path.join('.playwright-mcp', 'agent-configuration-error.png'), fullPage: true });
  } finally {
    await browser.close();
  }
}
