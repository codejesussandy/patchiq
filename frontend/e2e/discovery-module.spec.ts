...
test('Complete Discovery/Scanning Module Test', async ({ page }) => {
  ...
  try {
    ...
    if (await downloadButton.count() > 0) {
      ...
      const hasWindowsOption = await page.locator('text=/Windows/i').count() > 0;
      const hasMacOSOption = await page.locator('text=/MacOS/i').count() > 0 || await page.locator('text=/Mac/i').count() > 0;
      const hasLinuxOption = await page.locator('text=/Linux/i').count() > 0;
