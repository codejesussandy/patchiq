/**
 * Deploy and Test Windows Patches
 *
 * Deploys the latest version of each Windows app to Magical04 (Windows endpoint)
 * via bundle extraction path (agent downloads bundle.tar.gz, extracts, runs install script).
 *
 * Run: cd backend && npx tsx src/db/prisma/seeds/deploy-windows-patches.ts
 */
...
async function deployPatch(patch: {
  ...
  // Build payload — do NOT set 'script' for bundle-based patches (Path C)
...
async function main() {
  console.log('╔══════════════════════════════════════════════════════╗');
  console.log('║        Deploy & Test Windows Patches (Latest)        ║');
  console.log('╚══════════════════════════════════════════════════════╝\n');

  // Look up the Windows agent
  ...
  if (!agent) {
    console.error('ERROR: Windows agent not found:', WINDOWS_AGENT_ID);
    process.exit(1);
  }

  if (!agent.assetId) {
    console.error('ERROR: Windows agent has no linked asset');
    process.exit(1);
  }
  ...
  for (const software of WINDOWS_APPS) {
    // Find the latest bundled Windows patch for this app
    ...
    console.log(`          → ${agent.hostname} (WINDOWS, status: ${agent.status})`);
    ...
    try {
      ...
      await prisma.patch.update({
        ...
        data: {
          ...
          testNotes: outcome.success
            ? `Tested on ${agent.hostname} (WINDOWS): ${outcome.output?.slice(0, 200) || 'OK'}`
      ...
      // If latest passed, also mark ALL other versions of this app (same OS, testResult=null) as PASSED
      if (outcome.success) {
  ...
  // Summary
  console.log('\n╔═══════════════════════════════════════════════════════════════╗');
  console.log('║                     Windows Test Results                      ║');
  console.log('╠═══════════════════════════════════════════════════════════════╣');
  for (const r of results) {
    const line = `  ${r.status.padEnd(12)} ${r.patch.padEnd(50)}`;
